...
exports.storeLastEvent = functions.firestore
  .document('events/{eventId}')
  .onWrite((snap: any, context: any) => {
    return db
      .collection('dataForAutoCreate')
      .doc('events')
      .get()
      .then((doc: any) => {
        if (doc.data().lastEvent.seconds < snap.after.data().startTime.seconds) {
          return db
            .collection('dataForAutoCreate')
            .doc('events')
            .set({ lastEvent: snap.after.data().startTime });
        } else {
          return null;
        }
      });
