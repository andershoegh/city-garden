...
const Bed: React.FC<BedProps> = (props) => {
  ...
  let boxIllustration =
    content != null ? 'gardenbox-' + content + ' gardenbox' : 'gardenbox-empty gardenbox';
  const DOUBLE_CLICK_THRESHOLD = 300;
  const [touchStart, setTouchStart] = useState<number>(0);
  let message =
    'This garden box contains ' +
    content +
    '. <br> Click on okay to empty garden box ' +
    bedNr +
    '.';
  ...
  return (
    <div id={'garden-box-' + bedNr}>
      ...
      {availTasks !== 0 ? <IonBadge className='badge'> {availTasks} </IonBadge> : null}
    </div>
  );
};
export default Bed;
